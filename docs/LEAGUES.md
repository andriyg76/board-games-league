...
- Гравці з неактивними запрошеннями (pending або virtual статус) також включені в відповідь
- Гравець залишається доступним для вибору навіть якщо його запрошення скасоване або прострочене
...
- При скасуванні запрошення membership не видаляється - залишається як `pending` або `virtual`
- Гравець залишається доступним для вибору навіть після скасування запрошення
...
#### 13. Скасувати запрошення

**Endpoint:** `POST /api/leagues/{code}/invitations/{token}/cancel`

**Опис:** Скасовує активне запрошення. Тільки створювач запрошення може його скасувати.
...
**Статус коди:**
- `200 OK` - Запрошення успішно скасовано
- `400 Bad Request` - Недійсний токен або помилка при скасуванні
- `401 Unauthorized` - Відсутня або недійсна аутентифікація
- `403 Forbidden` - Користувач не є створювачем запрошення
- `404 Not Found` - Запрошення не знайдено
- `500 Internal Server Error` - Помилка сервера

**Поведінка:**
- Запрошення позначається як скасоване (expired)
- Якщо membership має ігри, він перетворюється на `virtual` статус
- Якщо membership не має ігор, він залишається як `pending` (не видаляється)
- Membership залишається доступним для вибору в новій грі навіть після скасування запрошення
- Зв'язок між membership та invitation очищається

**Примітки:**
- Скасоване запрошення автоматично з'являється в списку неактивних запрошень
- Гравець зі скасованим запрошенням залишається доступним для вибору в новій грі

---

#### 14. Продовжити запрошення

**Endpoint:** `POST /api/leagues/{code}/invitations/{token}/extend`

**Опис:** Продовжує термін дії неактивного (простроченого або скасованого) запрошення на 7 днів. Тільки створювач запрошення може його продовжити.
...
**Статус коди:**
- `200 OK` - Запрошення успішно продовжено
- `400 Bad Request` - Недійсний токен, запрошення вже використане, або помилка при продовженні
- `401 Unauthorized` - Відсутня або недійсна аутентифікація
- `403 Forbidden` - Користувач не є створювачем запрошення
- `404 Not Found` - Запрошення не знайдено
- `500 Internal Server Error` - Помилка сервера
...
**Поведінка:**
- Термін дії запрошення продовжується на 7 днів від поточного моменту
- Якщо membership існує і має статус `virtual`, він перетворюється на `pending`
- Membership пов'язується з переактивованим запрошенням
- Оновлюється `last_activity_at` для membership
- Переактивоване запрошення автоматично переміщується з неактивних в активні

**Примітки:**
- Можна продовжити тільки неактивні запрошення (прострочені або скасовані)
- Використані запрошення не можна продовжити
- Після переактивації гравець з'являється в списку доступних гравців для нової гри
