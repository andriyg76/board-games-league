...
**Важливо:** Операції створення, оновлення та видалення доступні тільки супер-адміністраторам.
...
- `optional` - необов'язково, будь-яка кількість (0+)
- `optional_one` - необов'язково, максимум один (0-1)
- `exactly_one` - рівно один гравець
- `required` - обов'язково хоча б один (1+)
- `multiple` - обов'язково кілька (2+)
- `moderator` - модератор гри (рівно 1)

---

### POST /api/game_types

Створює новий тип гри. **Тільки для супер-адміністраторів.**
...
**Відповідь:**
- Статус: 201 Created
- Тіло: Створений об'єкт типу гри

**Помилки:**
- 400 Bad Request: Недійсне тіло запиту або дублікат ключів ролей
- 401 Unauthorized: Недійсний або застарілий токен
- 403 Forbidden: Не є супер-адміністратором

**Примітки:**
- `scoring_type` повинен бути одним з: `classic`, `mafia`, `custom`, `cooperative`, `cooperative_with_moderator`, `team_vs_team`
- Ключі ролей повинні бути унікальними
...
Оновлює існуючий тип гри. **Тільки для супер-адміністраторів.**
...
Видаляє тип гри. **Тільки для супер-адміністраторів.**
...
**Помилки:**
- 400 Bad Request: Недійсний формат коду
- 403 Forbidden: Не є супер-адміністратором або тип гри є вбудованим (built_in)
- 404 Not Found: Тип гри не знайдено

**Примітки:**
- Вбудовані типи ігор (built_in: true) не можна видалити
...
- API геолокації (ipinfo.io): Перевірте документацію ipinfo.io для обмежень швидкості
...
## Точки доступу системи ліг

Для повної документації API системи ліг, включаючи endpoints для керування лігами, запрошень, таблиць лідерів та рекомендацій гравців, дивіться:

- **[LEAGUES.md](LEAGUES.md)** - Повна документація League System (включає API)
