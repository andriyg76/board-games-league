...
- Рекомендації гравців на основі історії спільних ігор (для ліг)

### Система рекомендацій гравців

Для ліг система надає інтелектуальні рекомендації гравців на основі історії спільних ігор:

- **Кеш останніх гравців**: Зберігається в `LeagueMembership.recent_co_players` (максимум 10 записів)
- **Оновлення кешу**: Автоматично оновлюється при фіналізації ігрового раунду
- **Сортування**: Гравці сортуються за датою останньої спільної гри (`last_played_at` DESC)
- **Активність**: Інші гравці сортуються за `last_activity_at` (NULLS LAST)

**API endpoint**: `GET /api/leagues/{code}/suggested-players`

Повертає три категорії:
- `current_player`: Поточний користувач (якщо є membership)
- `recent_players`: До 10 гравців з кешу останніх спільних ігор
- `other_players`: Інші члени ліги, сортовані за активністю
...
#### Логіка автозаповнення

При створенні нового раунду система автоматично заповнює слоти гравців на основі рекомендацій:
...
**Пріоритет заповнення:**
1. Поточний гравець (якщо є membership)
2. Останні спільні гравці (з кешу `recent_co_players`)
3. Інші члени ліги (сортовані за активністю)
...
### Ліги (рекомендації гравців)

| Метод | Точка доступу | Опис |
|-------|---------------|------|
| GET | `/api/leagues/{code}/suggested-players` | Отримати рекомендованих гравців для створення раунду |
| POST | `/api/leagues/{code}/invitations` | Створити інвайт (можна з alias для віртуального гравця) |
...
- Автозаповнення на основі рекомендованих гравців (`GET /api/leagues/{code}/suggested-players`)
- Підтримка модератора для відповідних ігор (можливість виділити гравця для ролі модератора)
- Валідація мінімальної/максимальної кількості
- Створення віртуальних гравців "на льоту" через інвайти
- Двопанельний інтерфейс: доступні гравці (зліва) та обрані гравці (справа)

**Потік вибору гравців:**
1. При завантаженні кроку викликається `GET /api/leagues/{code}/suggested-players`
2. Система автоматично заповнює слоти на основі пріоритету (поточний → recent → other)
3. Користувач може додавати/видаляти гравців вручну
4. Можна виділити одного гравця для призначення модератором (для ігор з модератором)
5. При створенні віртуального гравця він автоматично додається до обраних
...
Компонент двопанельного вибору гравців:
...
Діалог створення віртуального гравця:
- Поле вводу alias
- Перевірка унікальності (debounced) - через існуючу валідацію
- Кнопка "Створити"
- Використовує `POST /api/leagues/{code}/invitations` з alias
- При успіху: toast + копіювання посилання в буфер
- Автоматично додає до списку обраних
...
## Діаграма потоку створення раунду
