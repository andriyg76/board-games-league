...
- Використовує API ipinfo.io (потребує API токен)
...
- `IPINFO_TOKEN`: Токен для API ipinfo.io (для геолокації)
...
- `LOG_DIR`: Директорія для `server.log`, `access.log`, `debug.log` (якщо не задано — лог у консоль)
...
Система використовує API ipinfo.io для геолокації:
- Потребує токен API (`IPINFO_TOKEN`)
- Застосовуються обмеження швидкості (перевірте документацію ipinfo.io)
...
2. Переконайтеся що `IPINFO_TOKEN` налаштований
3. Переконайтеся що сервіс ipinfo.io доступний
4. Перевірте обмеження швидкості на ipinfo.io
